...
function toggleSolution(complaintId) {
    ...
    if (solutionElement.style.display === "none") {
        ...
        fetch(`${ROOT_URL}/public/workerHelpDesk/getConversation/${encodeURIComponent(complaintId)}`)
        .then((response) => {
            if (!response.ok) {
        ...
        .then((data) => {
            if (data && data.updates && data.updates.length > 0) {
                ...
                data.updates.forEach(update => {
                    ...
                    const updateClass = update.role === 'Worker' ? 'worker-update' : 
...
function initializeOverlayHandler() {
    ...
    closeBtn.addEventListener("click", function () {
        ...
        fetch(`${ROOT_URL}/public/workerHelpDesk/clearSessionMessage`)
            .then(response => {
                if (!response.ok) {
