document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadComplaintDetails(complaintId) {
        ...
        fetch(`${ROOT}/public/Complaint/details/${complaintId}`)
            .then(response => {
                if (!response.ok) {
    ...
    function loadChatHistory(complaintId) {
        fetch(`${ROOT}/public/Complaint/chat/${complaintId}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Server responded with status: ${response.status}`);
                }
    ...
    function sendReply(message) {
        ...
        // Send request
        fetch(`${ROOT}/public/Complaint/respond`, {
            method: 'POST',
            headers: {
    ...
    function deleteComplaint() {
        // Send request
        fetch(`${ROOT}/public/Complaint/delete`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
  ...
  document.getElementById('testButton').addEventListener('click', function () {
    const testId = 1;
    const url = `${ROOT}/public/Complaint/details/${testId}`;
    ...
    fetch(url)
      ...
      .then(({ text, status }) => {
        ...
        try {
          ...
          console.error('❌ Invalid JSON. Status:', status);
